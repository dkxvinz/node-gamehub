...
router.get("/", async (req, res) => {
  try {
    ...
    // ห่อ 'rows' ด้วย object { discount: ... }
    // เพื่อให้ตรงกับ Model 'DiscountGetRes'
...
router.get("/:discountId", async (req, res) => {
  try {
    ...
    const [rows] = await conn.query<RowDataPacket[]>(
      "SELECT discount_id,discount_code,discount_price,max_quantity,limit_user FROM discount WHERE  discount_id = ?",
      [did]
    );
    if (rows.length === 0) {
  ...
  try {
    if (!discountCode || !discountPrice || !maxQuantity) {
      return res
        ...
        .json({
          message:
            "please check discount code or max quantity or limit user are required",
        });
    }
    const sql =
      "INSERT INTO discount(discount_code,discount_price,max_quantity) VALUES (?,?,?)";
...
router.put("/update/:discountId", async (req, res) => {
  ...
  try {
    ...
    values.push(did);
    const sql = `UPDATE discount SET ${updates.join(
      ", "
    )} WHERE discount_id = ?`;
...
router.delete("/delete/:discId", async (req, res) => {
  ...
  try {
    ...
    if (header.affectedRows > 0) {
      ...
      console.log(
        `Attempted to delete non-existent game with ID ${discountId}.`
      );
      return res
        .status(404)
        .json({ message: `Game with ID ${discountId} not found.` });
    }
  } catch (error) {
